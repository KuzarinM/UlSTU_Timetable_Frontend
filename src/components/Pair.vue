	...
	export default{
		...
		methods:{
			...
			PairDivClassDefine(){
				this.divClass = 'p-1 '
				if(this.pairs!=null){
					if(this.pairs.some(x=>x.isDif != 1)) 
						this.divClass+= 'p-1 table-active '
				} 
				if(this.PairIsNowFunc(this.weekNumber, this.dayNumber, this.pairNumber)) 
					this.divClass += "current-day "
			...
			PrepareData(){
				if(!this.isEmpty){
					if(this.timetableType != "g"){
						this.data = []
						const tmp = this.pairs.filter(x=>x.isDif != this.excludedNumber).reduce((groups,item)=>{
		...
		mounted(){
			this.excludedNumber = this.differenceMod? 1 : 0

			this.isEmpty = 
				this.pairs == null 
				|| this.pairs.filter(x=>x.isDif != this.excludedNumber).length == 0
	...
	<td :class="this.divClass">
		<div class="p-0 m-0" v-if="!this.isEmpty" >
			<div class="p-0 m-0" v-for="(subject, si) in this.data">
				<h6 class="fw-bold m-0" v-if="!this.IsDesctop && si==0">
					{{ this.pairNumber +1 }} - {{ this.pairTiming[this.pairNumber] }}
				</h6>
				...
				<div class="m-0 p-0">
					<a 
						:href="p.replace('/','%2F')" 
						v-if="this.timetableType != 'p'" 
						v-for="p in subject.places"
					>
						<p :class="this.textClass">
						{{ p }}
						</p>
